			...
			if (buffer == null || buffer.isEmpty()) {
				...
				if (list.getCursorArgumentIndex() == 0) {
					...
					if (command.hasCompleter()) {
						 ...
						 * delim.isDelimiter( buffer, cursor ) ) { for ( int i =
						 * 0; i < candidates.size(); i++ ) { CharSequence val =
						 * candidates.get( i );
						 * 
						 * while ( val.length() > 0 && delim.isDelimiter( val,
						 * val.length() - 1 ) ) { val = val.subSequence( 0,
						 * val.length() - 1 ); }
						 * 
