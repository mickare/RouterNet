...
public class Router extends AbstractIdleService implements Owner {
	...
	protected Router( ConsoleReader console, Logger logger ) throws IOException {
		...
		this.scheduler = MoreExecutors.getExitingScheduledExecutorService( new ScheduledThreadPoolExecutor( 0, new ThreadFactoryBuilder().setNameFormat( "Router Pool Thread #%1$d" ).build() ), 10, TimeUnit.SECONDS );
		
		HomeNode home = new HomeNode( this.configFile.getConfig().getRouterSettings().getHome().getId(), this.getConfig().getRouterSettings().getHome().getNamespaces() );
	...
	protected void startUp() throws Exception {
		...
		this.address = HostAndPort//
				.fromString( this.configFile.getConfig().getRouterSettings().getAddress() ).withDefaultPort( 791 );
		
		this.authentication = AuthenticationFactory.newPasswordForRouter( this.getConfigFile().getConfig().getRouterSettings().getPassword() );
