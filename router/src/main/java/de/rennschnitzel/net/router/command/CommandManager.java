...
public class CommandManager {
	...
	public void dispatchCommand( String commandLine ) {
		...
		try {
			command.execute( cmdArgs.length == 1 ? new String[0] : Arrays.copyOfRange( cmdArgs, 1, cmdArgs.length ) );
		} catch ( Exception e ) {
	...
	private class CommandCompleter implements Completer {
		...
		public int complete( final String buffer, final int cursor, final List<CharSequence> candidates ) {
			...
			if ( buffer == null || buffer.isEmpty() ) {
				...
				ArgumentCompleter.ArgumentDelimiter delim = new ArgumentCompleter.WhitespaceArgumentDelimiter();
				...
				if ( list.getCursorArgumentIndex() == 0 ) {
					...
					if ( command.hasCompleter() ) {
						...
						// Do complete
						int ret = command.getCompleter().complete( String.join( " ", Arrays.copyOfRange( list.getArguments(), 1, list.getArguments().length ) ), newCursor, candidates );
						
						int pos = cmd.length() + 1 + ret;
						
						/*
						 * if ( ( cursor != buffer.length() ) && delim.isDelimiter( buffer, cursor ) ) { for ( int i =
						 * 0; i < candidates.size(); i++ ) { CharSequence val = candidates.get( i );
						 * 
