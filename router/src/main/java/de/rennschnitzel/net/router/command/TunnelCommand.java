...
public class TunnelCommand extends AbstractCommand {
	...
	public void execute( String[] args ) {
		...
		if ( subcmd.equals( "info" ) ) {
			...
			int maxIdLength = tunnels.stream().mapToInt( Tunnel::getId ).map( id -> Integer.toHexString( id ).length() ).max().orElse( 1 );
			int maxNameLength = tunnels.stream().map( Tunnel::getName ).mapToInt( String::length ).max().orElse( 1 );
