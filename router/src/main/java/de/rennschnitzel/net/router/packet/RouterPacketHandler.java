...
public class RouterPacketHandler extends BasePacketHandler {
			...
			if ( node != home && !sender.equals( node.getId() ) ) {
				...
				if ( out != null && out.isActive() ) {
					...
					if ( proc != null && !node.hasProcedure( proc ) ) {
						sendProcedureFail( in, msg, sender, node.getId(), ErrorMessage.newBuilder().setType( ErrorMessage.Type.UNDEFINED ).setMessage( "unregistered procedure" ) );
						continue;
					}
	...
	private void sendProcedureFailNotConnected( final Connection in, final ProcedureMessage msg, final UUID recever, final UUID sender ) {
		sendProcedureFail( in, msg, recever, sender, ErrorMessage.newBuilder().setType( ErrorMessage.Type.UNAVAILABLE ).setMessage( "not connected" ) );
	}
	
	private void sendProcedureFail( final Connection in, final ProcedureMessage msg, final UUID recever, final UUID sender, ErrorMessage.Builder error ) {
