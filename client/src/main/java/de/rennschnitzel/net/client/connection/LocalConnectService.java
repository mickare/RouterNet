...
public class LocalConnectService extends AbstractConnectService {
  ...
  protected LocalConnectClient newConnectClient(Promise<Connection> promise) {
    ...
    RouterLoginEngine engine_test =
        new RouterLoginEngine(framework.getRouterNetwork(), framework.getAuthenticationRouter());
    ChannelInitializer<LocalChannel> init_test = PipelineUtils.baseInitAnd(ch -> {
      ...
      ch.pipeline()
          .addLast(new ConnectionHandler(framework.getRouterNetwork(), BasePacketHandler.DEFAULT));
    });


    ClientLoginEngine engine_client =
