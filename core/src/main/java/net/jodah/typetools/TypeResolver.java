...
public final class TypeResolver {
	 ...
	 *             if more or less than one argument is resolved for the {@code type}
	 */
	 ...
	 * @throws IllegalArgumentException
	 *             if more or less than one argument is resolved for the {@code genericType}
	 */
	 ...
	 * @param subType
	 *            to extract type variable information from
	 * @return array of raw classes representing arguments for the {@code type} else {@code null} if no type arguments
	 *         are declared
	 */
	...
	/**
	 * Returns an array of raw classes representing arguments for the {@code genericType} using type variable
	...
	/**
	 * Returns the generic {@code type} using type variable information from the {@code subType} else {@code null} if
	 * the generic type cannot be resolved.
	 * 
	 * @param type
			...
			try {
				...
				for ( Method m : functionalInterface.getMethods() ) {
					if ( !m.isDefault() && !Modifier.isStatic( m.getModifiers() ) && !m.isBridge() ) {
						...
						String[] methodRefInfo = constantPool.getMemberRefInfoAt( constantPool.getSize() - resolveMethodRefOffset( constantPool, lambdaType ) );
						
						// Skip auto boxing methods
						if ( methodRefInfo[1].equals( "valueOf" ) && constantPool.getSize() > 22 ) {
							try {
								methodRefInfo = constantPool.getMemberRefInfoAt( constantPool.getSize() - resolveAutoboxedMethodRefOffset( constantPool, lambdaType ) );
	...
	/**
	 * Resolves method ref offset of method reference lambda type having autoboxed return type. We can't cache the
	 * result value because results are different depending on given {@code lambdaType}.
	 */
