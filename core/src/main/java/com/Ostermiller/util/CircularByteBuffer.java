/*
 * Circular Byte Buffer Copyright (C) 2002-2010 Stephen Ostermiller
...
/**
 * Implements the Circular Buffer producer/consumer model for bytes. More information about this
 ...
 * PipedInputStreams and PipedOutputStreams don't support the mark operation, don't allow you to
 * control buffer sizes that they use, and have a more complicated API that requires instantiating
...
public class CircularByteBuffer {
   ...
   * The circular buffer.
   * <p>
   * The actual capacity of the buffer is one less than the actual length of the buffer so that an
   * empty and a full buffer can be distinguished. An empty buffer will have the markPostion and the
   * writePosition equal to each other. A full buffer will have the writePosition one less than the
   * markPostion.
   * <p>
   * There are three important indexes into the buffer: The readPosition, the writePosition, and the
   * markPosition. If the InputStream has never been marked, the readPosition and the markPosition
   * should always be the same. The bytes available to be read go from the readPosition to the
   * writePosition, wrapping around the end of the buffer. The space available for writing goes from
   * the write position to one less than the markPosition, wrapping around the end of the buffer.
   * The bytes that have been saved to support a reset() of the InputStream go from markPosition to
  ...
  /**
   * True if a write to a full buffer should block until the buffer has room, false if the write
  ...
  /**
   * Make this buffer ready for reuse. The contents of the buffer will be cleared and the streams
   * associated with this buffer will be reopened if they had been closed.
   *
   * @since ostermillerutils 1.00.00
   */
  public void clear() {
   ...
   * <p>
   * Write methods may throw a BufferOverflowException if the buffer is not large enough. A large
   * enough buffer size must be chosen so that this does not happen or the caller must be prepared
   * to catch the exception and try again once part of the buffer has been consumed.
   *
   *
   ...
   * <p>
   * Note that the number of bytes available plus the number of bytes free may not add up to the
   * capacity of this buffer, as the buffer may reserve some space for other purposes.
   *
  ...
  /**
   * Create a new buffer with a default capacity. Writing to a full buffer will block until space is
   * available rather than throw an exception.
   *
   * @since ostermillerutils 1.00.00
   */
  ...
  /**
   * Create a new buffer with given capacity. Writing to a full buffer will block until space is
   * available rather than throw an exception.
   * <p>
   * Note that the buffer may reserve some bytes for special purposes and capacity number of bytes
   * may not be able to be written to the buffer.
   * <p>
   * Note that if the buffer is of INFINITE_SIZE it will neither block or throw exceptions, but
   ...
   *
   * @param blockingWrite true writing to a full buffer should block until space is available, false
   *        if an exception should be thrown instead.
   *
   * @since ostermillerutils 1.00.00
   */
  ...
  protected class CircularByteBufferInputStream extends InputStream {
    ...
    /**
     * Close the stream. Once a stream has been closed, further read(), available(), mark(), or
     * reset() invocations will throw an IOException. Closing a previously-closed stream, however,
     * has no effect.
     *
     ...
     * no effect.
     *
     * @param readAheadLimit Limit on the number of bytes that may be read while still preserving
     *        the mark. After reading this many bytes, attempting to reset the stream will fail.
     *
     * @since ostermillerutils 1.00.00
     */
    @Override
    public void mark(int readAheadLimit) {
      synchronized (CircularByteBuffer.this) {
        // if (inputStreamClosed) throw new IOException("InputStream has been closed; cannot mark a
        // closed InputStream.");
    ...
    /**
     * Read a single byte. This method will block until a byte is available, an I/O error occurs, or
     * the end of the stream is reached.
     *
     * @return The byte read, as an integer in the range 0 to 255 (0x00-0xff), or -1 if the end of
    ...
    /**
     * Read bytes into an array. This method will block until some input is available, an I/O error
    ...
    /**
     * Reset the stream. If the stream has been marked, then attempt to reposition i at the mark. If
     * the stream has not been marked, or more bytes than the readAheadLimit have been read, this
  ...
  /**
   * Class for writing to a circular byte buffer. If the buffer is full, the writes will either
   * block until there is some space available or throw an IOException based on the
  ...
  protected class CircularByteBufferOutputStream extends OutputStream {
    ...
    /**
     * Close the stream, flushing it first. This will cause the InputStream associated with this
     * circular buffer to read its last bytes once it empties the buffer. Once a stream has been
     * closed, further write() or flush() invocations will cause an IOException to be thrown.
    ...
    /**
     * Write an array of bytes. If the buffer allows blocking writes, this method will block until
     * all the data has been written rather than throw an IOException.
     *
     * @param cbuf Array of bytes to be written
     * @throws BufferOverflowException if buffer does not allow blocking writes and the buffer is
     *         full. If the exception is thrown, no data will have been written since the buffer was
    ...
    /**
     * Write a portion of an array of bytes. If the buffer allows blocking writes, this method will
    ...
    /**
     * Write a single byte. The byte to be written is contained in the 8 low-order bits of the given
     * integer value; the 24 high-order bits are ignored. If the buffer allows blocking writes, this
     * method will block until all the data has been written rather than throw an IOException.
     *
